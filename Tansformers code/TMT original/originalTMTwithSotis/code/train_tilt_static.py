...
def get_args():
    ...
    parser.add_argument('--patch-size', '-ps', dest='patch_size', type=int, default=190, help='Batch size')
    parser.add_argument('--val-period', '-vp', dest='val_period', type=int, default=3, help='number of frames used for reconstruction')
...
def main():
    ...
    logging.basicConfig(filename=f'{run_path}/recording.log', level=logging.INFO, format='%(levelname)s: %(message)s')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    get_cuda_info(logging)
    
    train_dataset = DataLoaderTurbImage(rgb_dir=args.train_path, num_frames=args.num_frames, total_frames=50, im_size=args.patch_size, is_train=True)
    ...
    val_dataset = DataLoaderTurbImage(rgb_dir=args.val_path, num_frames=args.num_frames, total_frames=50, im_size=args.patch_size, is_train=False)
    ...
    model = DetiltUNet3DS(norm='LN', conv_type='dw',residual='pool', noise=0.001).to(device)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        logging.info('Training: Epoch {:d}/{:d} -Time:{:.6f} -LR:{:.7f} -Loss {:8f} -PSNR: {:.2f} dB; SSIM: {:.4f}'.
                    format(epoch, epochs, time.time()-epoch_start_time, optimizer.param_groups[0]['lr'], epoch_loss/epoch_step, psnr, ssim))
