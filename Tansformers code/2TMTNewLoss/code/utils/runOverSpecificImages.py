...
model_script_path = os.path.abspath('../mixedTresMetrics_08-02-2024-14-37-24')
...
OUTPUT_ROOT = '/media/Data/sebasModels/BestModelsTurbulenceMitg/outputImages/2TMTwithNewLoss'
EXTENDED = 0
NUM_FRAMES = 12
# =====================
...
def process_sequence(seq_folder, model):
    ...
    turb_images = sorted([os.path.join(turb_folder, f)
                          ...
                          if f.endswith('.png') or f.endswith('.jpg')])
    if len(turb_images) < NUM_FRAMES:
        print(f"⚠️ Only {len(turb_images)} frames found in {turb_folder}, expected {NUM_FRAMES}")
        return None
    turb_images = turb_images[:NUM_FRAMES]

    # --- Load tensors ---
    imgs = [load_image_tensor(p).to(DEVICE) for p in turb_images]
    input_batch = torch.stack(imgs, dim=1)  # [1,12,1,H,W]
    gt_img = load_image_tensor(gt_path).to(DEVICE)

    # --- SSIM before ---
    ssim_before = np.mean([calc_ssim(x, gt_img) for x in imgs])
