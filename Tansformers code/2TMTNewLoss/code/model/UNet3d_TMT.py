...
class DetiltUNet3DS(nn.Module):
    ...
    def center_crop(self, layer, target_size):
        _, _, depth, height, width = layer.size()
        diff_z = (depth - target_size[0]) // 2
        diff_y = (height - target_size[1]) // 2
        diff_x = (width - target_size[2]) // 2
    ...
    def forward(self, x):
        ...
        # Decoder part
                ...
                # Decoder part seb
        deconv3 = self.deconv_blk3(base)
        #print(f"deconv3 shape: {deconv3.shape}")
        crop3 = self.center_crop(deconv3, x3.shape[2:])
        #print(f"x3 shape: {crop3.shape}")
        d3 = torch.cat([crop3, x3], dim=1)
        d_high3 = self.dec_conv_blk3(d3)
        
        crop2 = self.center_crop(self.deconv_blk2(d_high3), x2.shape[2:])
        #print(f"d3shape: {d3.shape}")
        #print(f"d_high3 shape: {d_high3.shape}")
        #print(f"crop2 shape: {crop2.shape}")
        #print(f"x2 shape: {x2.shape}")
        d2 = torch.cat([crop2, x2], dim=1)
        d_high2 = self.dec_conv_blk2(d2)

        crop1 = self.center_crop(self.deconv_blk1(d_high2), x1.shape[2:])
        d1 = torch.cat([crop1, x1], dim=1)
        d_high1 = self.dec_conv_blk1(d1)


        #print(f"deconv3 base shape: {self.deconv_blk3(base).shape}")
        #print(f"crop3  shape: {crop3.shape}")
        ...
        print(f"Flow size before permute: {self.upsample3(flow3).size()}")
        ...
        if conv_type == 'dw':
            self.deconv = nn.Sequential(
                ...
                nn.ConvTranspose3d(inp_feat, inp_feat, kernel_size=(kernel, kernel, kernel),
                                stride=(1, stride, stride), padding=(padding, padding-1, padding-1), 
