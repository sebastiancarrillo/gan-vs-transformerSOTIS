...
model_script_path = '../mixedTresMetrics_08-02-2024-14-37-24'

sys.path.append(model_script_path)

from scipts.TMT import TMT_MS
...
def sci_label(x):
    """Format float into compact scientific notation like 1e-17 or 3e-15"""
    if x == 0:
        return "0"
    exponent = int(np.floor(np.log10(abs(x))))
    mant = x / (10**exponent)
    # If mantissa is close to an integer, show integer mantissa (1, 3, ...)
    if abs(mant - round(mant)) < 1e-6:
        mant = int(round(mant))
        return f"{mant}e{exponent}"
    # otherwise show with one decimal
    return f"{mant:.1f}e{exponent}"

# --- Reemplazar la función plot_metric existente por esta versión ---
def plot_metric_window(dict1, dict2, metric_name, title, window_idx, extended):
    """
    Dibuja un único plot en su propia ventana (plt.figure).
    dict1, dict2 : dict-like mapping keys -> dict of metrics
    metric_name   : 'SSIM' o 'PSNR'
    window_idx    : índice (1..4) usado para nombrar la ventana / archivo
    extended      : 0/1 igual que antes para título
    """
    def to_float_keys(d):
        new = {}
        for k, v in d.items():
    ...
    d1 = to_float_keys(dict1)
    d2 = to_float_keys(dict2)

    # Decide ascendente/descendente según el índice (tu heurística)
    ascending = True if window_idx > 2 else False

    keys1 = sorted(d1.keys())
    keys2 = sorted(d2.keys())
    all_keys = sorted(set(keys1) | set(keys2), reverse=(not ascending))

    # Construir valores alineados con all_keys
    values1 = [d1.get(k, {}).get(metric_name, 0) for k in all_keys]
    values2 = [d2.get(k, {}).get(metric_name, 0) for k in all_keys]

    # --- AQUI: invertir el orden de las keys y sus valores para que el eje x quede invertido
    all_keys = all_keys[::-1]
    values1 = values1[::-1]
    values2 = values2[::-1]

    x_labels = [sci_label(k) for k in all_keys]

    if extended == 1:
        plot_title = "Extendido"
    elif extended == 0:
        plot_title = "Normal"
    else:
        plot_title = title
    ...
    plt.bar(x - width/2, values1, width=width, label='Reconstructed')
    plt.bar(x + width/2, values2, width=width, label='Turbulence')
    plt.xlabel('Keys')
    ...
    try:
        ...
        outname = f"plot_window_{window_idx}_{metric_name}.png"
        plt.savefig(outname, bbox_inches='tight')
        print(f"[INFO] Entorno sin display: guardada figura en {outname}")
...
def evaluate_model(model, device,extended):
    ...
    plot_metric_window(dictionario1, dictionario1Turb, 'SSIM', 'SSIM for L', 1, extended)
    plot_metric_window(dictionario1, dictionario1Turb, 'PSNR', 'PSNR for L', 2, extended)
    plot_metric_window(dictionario2, dictionario2Turb, 'SSIM', 'SSIM for Cn2', 3, extended)
    plot_metric_window(dictionario2, dictionario2Turb, 'PSNR', 'PSNR for Cn2', 4, extended)
...
def main():
    ...
    model_path = '/media/Data/sebasModels/BestModelsTurbulenceMitg/2TMTwithNewLoss/mixedTresMetrics_08-02-2024-14-37-24/checkpoints/model_best.pth'
    extended=0
    model = load_model(model_path, device,extended)
    #model =1
    print("TMT")
