...
def get_args():
    ...
    parser.add_argument('--patch-size', '-ps', dest='patch_size', type=int, default=140, help='Batch size')
...
def main():
    ...
    ######### Scheduler ###########
    epochs = args.epochs
    start_epoch = 7 ##original 1
    warmup_epochs = 1  ##original 3
    ...
    ''')
    print("trining size", len(train_dataset))
        ...
        for epoch_step, data in enumerate(train_loader):
            ...
            for b in range(output.shape[0]):
                for i in range(output.shape[1]):
                    target_frame = target[b, i, ...]
            
                    # Verificar si el frame target está lleno de ceros
                    if torch.all(target_frame == 0):
                        print(f"Frame problemático encontrado en batch {epoch_step}, sample {b}, frame {i}, saltando este frame")
                        continue
                    ...
                    #print("ssim",util.calculate_ssim(img, img_gt, border=0))
