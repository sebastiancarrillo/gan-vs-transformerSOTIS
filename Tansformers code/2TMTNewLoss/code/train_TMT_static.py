...
class SsimLoss(nn.Module):
    ...
    def forward(self, prediction, target):
        ...
        #ssim_loss_powered = torch.pow(10, ssim_loss)
        return ssim_loss
...
class ExtendedModel(nn.Module):
    def __init__(self, original_model):
        ...
        # Integrar TinyTMT_MS como la nueva capa
        self.new_layers = DynamicInputNetwork()
    def forward(self, x, input_layer):

        x = self.original_model(x)  # Pasar la entrada por el modelo original
        x = self.new_layers(x, input_layer)  # Pasar la salida por las nuevas capas dinámicas
        #print("---------",input_layer)
        #self.new_layers.check_frozen_layers()
        
        return x
def inptLayerExtNetwork(img,img_gt):
    ssimM = tmf_ssim(img, img_gt, data_range=1.0)
...
def main():
    ...
    #criterion_edge = losses.EdgeLoss2D()
    ...
    for epoch in range(0,100000):
        ...
        for data in train_loader:
            ...
            #i want to trin the model first to deal with tilt and then the blur (in this code i dont change anything to the model befor training for blur) the difference is that turb cointainsbothtilt and blur, but blur contains only blur
            
            #if iter_count<=5000:
             #   input_ = data[1].cuda() #'turb':
            #else:
            #    input_ = data[0].cuda() #'blur'
            #    FolderUsed = 'blur'
            #    args.task = 'blur'
            
            #is better to use only turb because i need the network to learn the whole thing(tilt and blur)
            input_ = data[1].cuda() #'turb':
            ...
            #loss = criterion_char(output, target)#original
            # loss = criterion_char(output, target) + 0.05*criterion_edge(output, target)
            #loss = criterion_char(output, target)*0.3 + 0.7*criterionSsim(output, target)
            #loss = criterionSsim(output, target)
