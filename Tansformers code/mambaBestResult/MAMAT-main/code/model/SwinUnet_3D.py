...
class CyclicShift3D(nn.Module):
    def __init__(self, displacement):
        ...
        assert (
            type(displacement) is int or len(displacement) == 3
        ), f"displacement must be 1 or 3 dimension"
    ...
    def forward(self, x):
        return torch.roll(
            x,
            shifts=(self.displacement[0], self.displacement[1], self.displacement[2]),
            dims=(1, 2, 3),
        )
...
def create_mask3D(
    ...
    displacement: Union[int, List[int]],
    x_shift: bool,
    y_shift: bool,
    z_shift: bool,
):
    assert (
        type(window_size) is int or len(window_size) == 3
    ), f"window_size must be 1 or 3 dimension"
    ...
    for i in range(len(window_size)):
        assert 0 < displacement[i] < window_size[i], f"Error in Displacement Size"
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shifted:
            ...
            dots = rearrange(
                dots, "b h (n_x n_y n_z) i j -> b h n_y n_z n_x i j", n_x=nw_x, n_y=nw_y
            )
            dots[:, :, :, :, -1] += self.x_mask
...
class ConvBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.net = nn.Sequential(
            ...
            nn.PReLU(),
            nn.Conv3d(
                out_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=groups
            ),
            Norm(dim=out_ch),
            nn.PReLU(),
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.patch_expand = PatchExpanding3D(
            in_dim=in_dims, out_dim=out_dims, up_scaling_factor=up_scaling_factor
        )
...
class SwinUnet3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        num_classes=2,
        head_dim=32,
        window_size: Union[int, List[int]] = 2,
        downscaling_factors=(4, 2, 2, 2),
        relative_pos_embedding=True,
        dropout: float = 0.0,
        skip_style="stack",
        stl_channels: int = 32,
    ):  # second_to_last_channels
        super().__init__()
    ...
    def forward(self, img):
        ...
        assert x_s % (x_ws * 32) == 0, f"x_window_size*32"
        assert y_s % (y_ws * 32) == 0, f"y_window_size*32"
        assert z_s % (z_ws * 32) == 0, f"z_window_size*32"
