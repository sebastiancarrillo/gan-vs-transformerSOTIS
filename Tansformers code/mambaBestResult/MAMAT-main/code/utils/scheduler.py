...
class GradualWarmupScheduler(_LRScheduler):
    ...
    def step_ReduceLROnPlateau(self, metrics, epoch=None):
        """Reduce Steps of Optimizer

        Args:
            metrics: metrics to step
...
class CosineDecayWithWarmUpScheduler(_LRScheduler):
    ...
    def step(self):
        ...
        if self.warm_up:
            lr = (
                self.init_warmup_lr
                + (self.max_lr - self.init_warmup_lr) / self.warm_up_steps * self.iters
            )
        else:
            ...
            if self.iters <= self.num_step_down * T_cur:
                lr = (
                    ...
                    * (
                        1
                        + math.cos(math.pi * self.iters / (self.num_step_down * T_cur))
                    )
                    / 2
                )
                if lr < self.min_lr:
