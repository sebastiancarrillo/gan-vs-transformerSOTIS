...
#new network extended version
class TinyTMT_MSTransformers(nn.Module):
    def __init__(self, 
                 inp_channels=1,
                 out_channels=1, 
                 dim=16,  # Reduced for a smaller version
                 num_blocks=[1, 1],  # Reduced number of transformer blocks
                 num_refinement_blocks=1,
                 heads=[1, 2],  # Attention heads
                 ffn_expansion_factor=2.0,
                 att_type='shuffle',  # Using shuffle attention
                 att_ckpt=False,
                 bias=False,
                 ffn_ckpt=False,
                 LN_bias=True,
                 n_frames=5,  # Reduced number of frames for efficiency
                 out_residual=True):
        super(TinyTMT_MSTransformers, self).__init__()

        # Preprocessing layer
        self.getFeature = nn.Conv3d(inp_channels, dim, kernel_size=3, padding=1)
        
        # First transformer block sequence
        self.t1 = nn.Sequential(*make_level_blk(dim=dim, num_tb=num_blocks[0], nhead=heads[0], att=att_type,
                                                ffn=ffn_expansion_factor, bias=bias, LN_bias=LN_bias, 
                                                att_ckpt=att_ckpt, ffn_ckpt=ffn_ckpt, n_frames=n_frames))
        
        # Refinement layer
        self.refinement = nn.Conv3d(dim, out_channels, kernel_size=3, padding=1)

        # Residual connection flag
        self.out_residual = out_residual

    def forward(self, inp_img):
        # Initial features using Conv3D
        x = self.getFeature(inp_img)
        
        # Pass through transformer block
        x = self.t1(x)
        
        # Final refinement stage
        out_refined = self.refinement(x)
        
        # Optional residual connection
        if self.out_residual:
            out = out_refined + inp_img
        else:
            out = out_refined
        
        return out
class DynamicInputNetwork(nn.Module):
    def __init__(self):
        super(DynamicInputNetwork, self).__init__()
        #self.layer1 = TMT_MS( num_blocks=[1,1,1,1], num_refinement_blocks=1, n_frames=12, att_type='shuffle' )
        #self.layer2 = TMT_MS( num_blocks=[1,1,1,1], num_refinement_blocks=1, n_frames=12, att_type='shuffle' )
        #self.layer3 = TMT_MS( num_blocks=[1,1,1,1], num_refinement_blocks=1, n_frames=12, att_type='shuffle' )
        #self.layer4 = TMT_MS( num_blocks=[1,1,1,1], num_refinement_blocks=1, n_frames=12, att_type='shuffle' )
        self.layer1 = ExtendedTinyTMT_MS()
        self.layer2 = ExtendedTinyTMT_MS()
        self.layer3 = ExtendedTinyTMT_MS()
        self.layer4 = ExtendedTinyTMT_MS()
        self.relu = nn.ReLU()
    
    def forward(self, x, input_layer):
        # Descongelar todas las capas primero
        
        #self.freeze_layers(input_layer)
    ...
    def freeze_layers(self, input_layer):
        
        for c, layer in enumerate([self.layer1, self.layer2, self.layer3, self.layer4]):
            if input_layer <= c:
    ...
    def check_frozen_layers(self):
        for c, layer in enumerate([self.layer1, self.layer2, self.layer3, self.layer4]):
            for name, param in layer.named_parameters():
                print(f"Capa {c} - {name} - requires_grad: {param.requires_grad}")

class ExtendedTinyTMT_MS(nn.Module):
    def __init__(self, 
                 inp_channels=1,
                 out_channels=1, 
                 dim=32,  # Increased for more complexity
                 num_blocks=[2, 2],  # Increased number of blocks
                 num_refinement_blocks=2,  # Additional refinement blocks
                 heads=[2, 4],  # Increased heads for more attention
                 ffn_expansion_factor=2.5,  # Larger expansion factor
                 bias=False,
                 LN_bias=True,
                 n_frames=5,  
                 out_residual=True):
        super(ExtendedTinyTMT_MS, self).__init__()
        
        self.out_residual = out_residual
        
        # Preprocessing block with more layers
        self.getFeature = nn.Sequential(
            nn.Conv3d(inp_channels, dim, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(dim, dim, kernel_size=3, padding=1)
        )
        
        # Encoding block with additional layers
        self.encode = nn.Sequential(
            nn.Conv3d(dim, dim * 2, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(dim * 2, dim * 4, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(dim * 4, dim * 8, kernel_size=3, padding=1)  # Added complexity
        )
        
        # Decoding block with skip connections for residual learning
        self.decode = nn.Sequential(
            nn.Conv3d(dim * 8, dim * 4, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(dim * 4, dim * 2, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(dim * 2, dim, kernel_size=3, padding=1)
        )
        
        # Refinement block with residual enhancement
        ...
        # Refinement blocks for additional detail recovery
        self.extra_refinement = nn.Sequential(
    ...
    def forward(self, inp_img):
        inp_enc = self.getFeature(inp_img)
        out_enc = self.encode(inp_enc)
        out_dec = self.decode(out_enc)
        out_refined = self.refinement(out_dec)
        
        # Apply additional refinement blocks
        out_refined = self.extra_refinement(out_refined)
...
##new network extended version
class TinyTMT_MS(nn.Module):
    def __init__(self, 
                 inp_channels=1,
                 out_channels=1, 
                 dim=16,  # Reducido para una versión más pequeña
                 num_blocks=[1,1],  # Reducir la cantidad de bloques
                 num_refinement_blocks=1,
                 heads=[1,2],
                 ffn_expansion_factor=2.0,
                 bias=False,
                 LN_bias=True,
                 n_frames=5,  # Reducir la cantidad de frames
                 out_residual=True):
        super(TinyTMT_MS, self).__init__()
...
class Generator(nn.Module):
    def __init__(self, 
        ...
        self.out_residual = out_residual
        # Preprocessing: Convert the input from 1 channel to 16 channels using Conv3d
        self.getFeature = nn.Conv3d(in_channels, dim, kernel_size=3, padding=1)
        
        
        # Initial Layer (ConvTranspose3d, now it expects 16 channels from getFeature)
        self.initial = nn.ConvTranspose3d(dim, dim, kernel_size=3, stride=(3, 1, 1), padding=1)
        self.relu = nn.ReLU(inplace=True)
        
        # Residual blocks using ConvTranspose3d
        self.res_block_trans1 = ResBlockTrans3D(dim, use_dropout=True)
        self.res_block_trans2 = ResBlockTrans3D(dim, use_dropout=True)

        # Additional layers (ConvTranspose3d)
        self.conv_trans1 = nn.ConvTranspose3d(dim, dim, kernel_size=3, stride=(2, 1, 1), padding=1)
        self.conv_trans2 = nn.ConvTranspose3d(dim, dim*2, kernel_size=3, stride=(2, 1, 1), padding=1)
        self.conv_trans3 = nn.ConvTranspose3d(dim*2, dim*4, kernel_size=3, stride=1, padding=1)
        self.conv_trans4 = nn.ConvTranspose3d(dim*4, dim*2, kernel_size=3, stride=1, padding=1)

        # Output Layer (Conv3d)
        self.output_layer = nn.Conv3d(dim*2, out_channels, kernel_size=3, padding=1)
    def forward(self, inp_img):
        # Input shape: [batch_size, channels, frames, height, width]
        #print(inp_img.shape)
        # First, increase the number of channels to 16
        x = self.getFeature(inp_img)
        ...
        x = self.res_block_trans1(x)
        x = self.conv_trans1(x)
        x = self.relu(x)
    
        x = self.res_block_trans2(x)
        ...
        x = F.interpolate(x, size=inp_img.shape[2:], mode='trilinear', align_corners=False)
        out = x + inp_img
      

        return out

# Residual Block with ConvTranspose3d
class ResBlockTrans3D(nn.Module):
    def __init__(self, filters, kernel_size=3, strides=1, use_dropout=False):
        super(ResBlockTrans3D, self).__init__()
        self.conv1 = nn.ConvTranspose3d(in_channels=filters, out_channels=filters*2, kernel_size=kernel_size, stride=strides, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.ConvTranspose3d(in_channels=filters*2, out_channels=filters, kernel_size=kernel_size, stride=strides, padding=1)
...
class ResBlock3D(nn.Module):
    def __init__(self, filters, kernel_size=3, strides=1, use_dropout=False):
        ...
        self.conv1 = nn.Conv3d(in_channels=filters, out_channels=filters*2, kernel_size=kernel_size, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv3d(in_channels=filters*2, out_channels=filters, kernel_size=kernel_size, padding=1)
        self.use_dropout = use_dropout
        if use_dropout:
            self.dropout = nn.Dropout(0.3)
